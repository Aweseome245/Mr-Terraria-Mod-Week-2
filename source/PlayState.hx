...
class PlayState extends MusicBeatState
{
	...
	public var endDiaFinished:Bool;
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'thorns':
					curStage = 'schoolEvil';
				case 'welcome-back' | 'the-chill-zone' | 'later-nerd' | 'true-hell':
					curStage = 'terrariastage';
				case 'slightly-hell':
					curStage = 'adstage';
				default:
					curStage = 'stage';
		...
		switch (curStage)
		{
			...
			case 'terrariastage': //terraria crap
				var bg:BGSprite = new BGSprite('terrariastage', -600, -140, 0.9, 0.9);
				bg.scale.set(1.4, 1.4); // sprite stuff because Some Kid made the bg too small
				add(bg);
			case 'adstage': //ameridad stage
				var bg:BGSprite = new BGSprite('adstage', -600, -200, 0.9, 0.9); //leave it as it is, its pretty funny lmao
				add(bg);
			case 'spooky': //Week 2
				if(!ClientPrefs.lowQuality) {
		...
		var file:String = Paths.json(songName + '/dialogueEnd'); //Checks for json/Psych Engine dialogue at the end of the week. Just a copy of above with end.
		if (OpenFlAssets.exists(file)) {
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'welcome-back' | 'the-chill-zone' | 'later-nerd' | 'slightly-hell':
	...
	// the dialogue function for the end of weeks, since dialogues dont pause everything like they do on kade engine. Frick you, ShadowMario, and thank you BBPanzu.
	// nvm none of this works. i have tried so many methods wtf
	// if anyone can get it working DM me on discord Aweseome245#7012
	// until then end of week dialogues arent gonna happen
	public function startDialogueEnd(dialogueFile:DialogueFile, ?song:String = null):Void
		{
			endDiaFinished = false;
			// TO DO: Make this more flexible, maybe?
			if(dialogueFile.dialogue.length > 0) {
				trace(dialogueFile.dialogue.length);
				inCutscene = true;
				CoolUtil.precacheSound('dialogue');
				CoolUtil.precacheSound('dialogueClose');
				var doof:DialogueBoxPsych = new DialogueBoxPsych(dialogueFile, song);
	...
	function endStuff() // thanks bbpanzu lmao
	{
		FlxG.sound.playMusic(Paths.music('freakyMenu'));
